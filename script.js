...
async function createShareImage() {
  ...
  ctx.fillText(name, W * 0.5, H * 0.318);

  ctx.font = `bold ${Math.round(H * 0.016)}px Arial`;
  ctx.fillText('Tempo', W * 0.307, H * 0.391);
  ctx.fillText('Tentativas', W * 0.694, H * 0.391);

  ctx.font = `bold ${Math.round(H * 0.038)}px Arial`;
  ctx.fillText(formatTime(gameSeconds), W * 0.307, H * 0.490);
  ctx.fillText(String(attempts).padStart(2, '0'), W * 0.694, H * 0.490);
}
